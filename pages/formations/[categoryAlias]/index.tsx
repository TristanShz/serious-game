...
export const CategoryAlias = (props: { formations: TFormationMdl[]; category: TCategoryMdl }) => {
    ...
    return (
        <>
            <Meta
                url={basePath}
                title={`Metz Numeric Game : Retrouvez la liste de nos formations en ${props.category.blockTitle} `}
                description={""}
            />
...
export const getStaticProps: GetStaticProps = async (context) => {
    if (context.params && typeof context.params.categoryAlias === "string") {
        ...
        const { data: category } = await categoriesStore.getOneByAlias(alias);
